...
const ar_coordinate_convert_1 = require("../src/ar-coordinate-convert");
describe('AR Coordinate Convert', () => {
    describe('solarToWorld', () => {
        it('converts horizon (0 altitude)', () => {
            const radius = 50;
            let pos = (0, ar_coordinate_convert_1.solarToWorld)(0, 0, radius); // north
            expect(pos.x).toBeCloseTo(0);
            expect(pos.y).toBeCloseTo(0);
            expect(pos.z).toBeCloseTo(-radius);
            pos = (0, ar_coordinate_convert_1.solarToWorld)(90, 0, radius); // east
            expect(pos.x).toBeCloseTo(radius);
            expect(pos.y).toBeCloseTo(0);
            expect(pos.z).toBeCloseTo(0);
            pos = (0, ar_coordinate_convert_1.solarToWorld)(180, 0, radius); // south
            expect(pos.x).toBeCloseTo(0);
            expect(pos.y).toBeCloseTo(0);
            expect(pos.z).toBeCloseTo(radius);
            pos = (0, ar_coordinate_convert_1.solarToWorld)(270, 0, radius); // west
            expect(pos.x).toBeCloseTo(-radius);
            expect(pos.y).toBeCloseTo(0);
            expect(pos.z).toBeCloseTo(0);
        });
        it('converts zenith (90 altitude)', () => {
            const radius = 50;
            const pos = (0, ar_coordinate_convert_1.solarToWorld)(0, 90, radius);
            expect(pos.x).toBeCloseTo(0);
    ...
    describe('worldToSolar (round-trip)', () => {
        ...
        testCases.forEach(({ azimuth, altitude }) => {
            it(`round trips azimuth ${azimuth}, altitude ${altitude}`, () => {
                const radius = 50;
                const { x, y, z } = (0, ar_coordinate_convert_1.solarToWorld)(azimuth, altitude, radius);
                const result = (0, ar_coordinate_convert_1.worldToSolar)(x, y, z);
    ...
    describe('solarToWorld — negative altitude (below horizon)', () => {
        it('negative altitude produces y < 0', () => {
            const pos = (0, ar_coordinate_convert_1.solarToWorld)(180, -30, 50);
            expect(pos.y).toBeLessThan(0);
        });
        it('altitude=-90 (nadir) puts point directly below', () => {
            const pos = (0, ar_coordinate_convert_1.solarToWorld)(0, -90, 50);
    ...
    describe('solarToWorld — default radius', () => {
        it('default radius is 50 when not specified', () => {
            const pos = (0, ar_coordinate_convert_1.solarToWorld)(90, 0); // east at horizon
            expect(pos.x).toBeCloseTo(50);
        });
    });
    describe('solarToWorld — known value at 45°/45°', () => {
        it('azimuth=45° altitude=45° radius=1 → known coordinates', () => {
            const pos = (0, ar_coordinate_convert_1.solarToWorld)(45, 45, 1);
    ...
    describe('worldToSolar — degenerate origin', () => {
        it('(0,0,0) returns {0, 0} as fallback', () => {
            const result = (0, ar_coordinate_convert_1.worldToSolar)(0, 0, 0);
    ...
    describe('solarToWorld — radius scaling', () => {
        it('doubling radius doubles all coordinates', () => {
            const p1 = (0, ar_coordinate_convert_1.solarToWorld)(135, 30, 10);
            const p2 = (0, ar_coordinate_convert_1.solarToWorld)(135, 30, 20);
