...
sunscope/
  ...
  PROGRESS.md               # cross-session tracking: done, blocked, next
  ...
  packages/
    sky-detection/           # BUILT FIRST: hardest module
      ...
      src/
        ...
        hemisphere-stitcher.ts
        arc-integrator.ts
        sky-classifier.ts    # interface for DeepLabV3 bridge (impl is native)
      __tests__/
    ...
    core/                    # BUILT THIRD: conforms to contracts from above
      ...
      src/
        ...
        timezone.ts          # UTC internals, display conversion
...
## CONVENTIONS.md (create this file first)
...
If Stitch MCP is unavailable, the agent should: generate the prompt text for each screen, save to `design-reference/prompts/`, and note in PROGRESS.md that manual generation at stitch.withgoogle.com is needed.
